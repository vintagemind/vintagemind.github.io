[⬅️ Back](https://vintagemind.github.io/blog/)


## Useful Firefox Articles and Discussions

This post is really for my own reference but it may be useful to you:


[Firefox Support troubleshooting guide](https://support.mozilla.org/en-US/kb/firefox-support-troubleshooting-guide)



[Install an older version of Firefox](https://support.mozilla.org/en-US/kb/install-older-version-firefox)



[Secure connection failed and Firefox did not connect](https://support.mozilla.org/en-US/kb/secure-connection-failed-firefox-did-not-connect)




[Refresh Firefox - reset add-ons and settings](https://support.mozilla.org/en-US/kb/refresh-firefox-reset-add-ons-and-settings)




[I'm having problems confirming my Firefox Account](https://support.mozilla.org/en-US/kb/im-having-problems-confirming-my-firefox-account)




[Quick fixes if your Firefox slowsdown](https://support.mozilla.org/en-US/kb/quick-fixes-if-your-firefox-slows-down)





[Websites look wrong or appear differently than they should](https://support.mozilla.org/en-US/kb/websites-look-wrong-or-appear-differently)




[Websites show a spinning wheel and never finish loading](https://support.mozilla.org/en-US/kb/websites-show-spinning-wheel-never-finish-load)




[Always check that Firefox is my default browser](https://support.mozilla.org/en-US/questions/1246986#answer-1189196)



[Restore the Menu bar in Firefox](https://support.mozilla.org/en-US/kb/restore-menu-bar-firefox)




[Total Cookie Protection and website breakage FAQ](https://support.mozilla.org/en-US/kb/total-cookie-protection-and-website-breakage-faq)




[How to restore a browsing session from backup](https://support.mozilla.org/en-US/kb/how-restore-browsing-session-backup)




[Enhanced Tracking Protection in Firefox for desktop](https://support.mozilla.org/en-US/kb/enhanced-tracking-protection-firefox-desktop)




[How to troubleshoot time related errors on secure websites](https://support.mozilla.org/en-US/kb/troubleshoot-time-errors-secure-websites)




[Firefox DNS-over-HTTPS](https://support.mozilla.org/en-US/kb/firefox-dns-over-https)
